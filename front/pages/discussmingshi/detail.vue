...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussmingshi');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','discussmingshi');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('discussmingshi', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussdiscussmingshi', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdiscussmingshi/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('discussmingshi', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('discussmingshi', this.detail);
				this.$utils.msg('审核成功');
