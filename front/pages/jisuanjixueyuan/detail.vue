<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
			...
			<view class="time-content" :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","margin":"0 0 2rpx 0","borderColor":"#fff","backgroundColor":"#fff","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap() {
				uni.setStorageSync('paytable', 'jisuanjixueyuan');
			...
			onAcrossTap(tableName) {
				uni.setStorageSync('crossTable', 'jisuanjixueyuan');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init() {
				let res = await this.$api.info('jisuanjixueyuan', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussjisuanjixueyuan/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('jisuanjixueyuan', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('jisuanjixueyuan', this.detail);
				this.$utils.msg('审核成功');
